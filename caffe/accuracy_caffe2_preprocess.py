...
def rescale(img, input_height, input_width):
	...
	#print(f"Model's input shape is {input_height}x{input_width}")
...
mean=m.mean(1).mean(1)
mean = mean[:, np.newaxis, np.newaxis]
def caffe2_preprocess_image(im_n):
	im_n=str(im_n).zfill(8)
	im_n='ILSVRC2012_val_'+im_n+'.JPEG'
	print(im_n)
	im=img_dir+'/ILSVRC2012_img_val/'+im_n
	img = skimage.img_as_float(skimage.io.imread(im)).astype(np.float32)
...
def read_image_i(im_n):
	...
	if len(img.shape)==2:
		...
		gray_c=gray_c+1
		img = np.stack((img,)*3, axis=-1)
	img=resize_image(img,[227,227],1)
...
def read_image_center_crop(im_n):
	...
	im_n='ILSVRC2012_val_'+im_n+'.JPEG'
	print(im_n)
	im='/home/ehsan/UvA/ARMCL/Khadas/ARMCL-Local/scripts/blobs_extractor/Working_tree/Accuracy/Imagenet/ILSVRC2012_img_val/'+im_n
	nh, nw = 224, 224
...
for indx in range(batch_size+1,n+batch_size+1,batch_size):
	...
	for j in range(last_i,indx):
		...
		##net.blobs['data'].data[local_indx] = caffe2_preprocess_image(j)
